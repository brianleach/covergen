 ...
 * checkout rather than rolled back, so the next move is a human one, and the
 * cheapest version of it is the command that turns the journal into the PR the
 * run would have opened. Nothing is regenerated by it.
 */
export function resumeNote(accepted: number, journal: string | undefined, signal: string): string {
  ...
  return `${head}Open the draft PR for them without regenerating:\n  covergen pr --from ${journal}\n`;
}
...
export function buildProgram(): Command {
  ...
  program
    .command("pr")
    .description("Open the draft PR for a run that ended without one, from the journal it left behind")
    .requiredOption("--from <path>", "run journal, `<state dir>/runs/<id>.json` in the target repo")
    .option("--pr-max-lines <n>", "override sweep.pr_max_lines for this PR")
    .action(async (opts: { from: string; prMaxLines?: string }) => {
      const { config, log } = context(program);
      // The journal names its own repo, so the config entry is looked up rather
      // than asked for: a --repo that disagreed with the journal would open a PR
      // in the wrong checkout.
      const journal = await readJournal(resolve(opts.from));
      const repo = findRepo(config, journal.repo);
      const urls = await openPrFromJournal({
        config,
        repo,
        journal,
        log,
        prMaxLines: parseCeiling(opts.prMaxLines, "--pr-max-lines", config.sweep.pr_max_lines),
      });
